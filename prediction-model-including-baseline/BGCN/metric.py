...
class NDCG(_Metric):
    ...
    def IDCG(self, num_pos):
        hit = torch.zeros(self.topk, dtype=torch.float,device=torch.device('cuda'))
        hit[:num_pos] = 1
        return self.DCG(hit)
    ...
    def __init__(self, topk):
        super().__init__()
        self.topk = topk
        self.IDCGs = torch.empty(1 + self.topk, dtype=torch.float,device=torch.device('cuda'))
...
class MRR(_Metric):
    ...
    def __init__(self, topk):
        super().__init__()
        self.topk = topk
        self.denominator = torch.arange(1, self.topk+1, dtype=torch.float,device=torch.device('cuda'))
